	...
	template <class Key, class T, class Compare = ft::less<Key>, class Allocator = std::allocator<ft::pair<const Key, T> > >
		class map
		{
			...
			public:
				...
				explicit map (const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _alloc(alloc), _comp(comp), _size(0),  _root() {}

				template <class InputIterator>
					map(InputIterator first, InputIterator last, const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _alloc(alloc), _comp(comp), _size(0), _root()
				{
					while (first != last)
					{
						_root = insertNode(_root, ft::make_pair(first->first, first->second));
						++_size;
						++first;
					}
				}

				map(const map& x) : _alloc(x._alloc), _comp(x._comp), _size(0), _root()
			{
				*this = x;
			}
				...
				mapped_type&	operator[](const key_type & k)
				{
					...
					ft::Node<value_type>*	node = _root.find_value(p);
					if (!node)
					{
						_root = insertNode(_root, ft::make_pair(k, mapped_type()));
						return (_root.find_value(p)->data.second);
					}
				...
				ft::pair<iterator, bool>	insert(const value_type& val)
				{
					...
					if (_root = insertNode(_root, val))
						ret.second = true;
					iterator it = _root.find_value(val);
					ret.first = it;
					return (ret);
				}
				...
				iterator	insert(iterator position, const value_type& val)
				{
					...
					if (ret.second)
						...
						return (it = _root.find_value(val));
				}
				...
				{
					_root = removeNode(_root, *position);
				}
				...
				size_type	erase(const key_type& k)
				{
					...
					if (node)
					{
						return (_root = removeNode(_root, node->data));
					}
